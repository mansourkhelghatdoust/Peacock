...
public class EagleJobArrivalEvent extends JobArrivalEvent {

	public EagleJobArrivalEvent(double time, double interarrivalDelay) {
	...
	private void scheduleLongJob(Job job, double currentTime) {
		...
		for (int i = 0; i < numOfTasks; i++) {
			...
			int workerId = EagleCentralizedScheduler.getWorkerId(job);
			
			EagleCentralizedScheduler.setOn(workerId);
			
			Simulation.addEvent(new EagleProbeEvent(currentTime, Workers.getWorker(EagleWorker.class, workerId), job,
					EagleCentralizedScheduler.getState()));
	...
	protected JobArrivalEvent getJobArrivalEvent() {
		...
		return new EagleJobArrivalEvent(Math.round(time + SimulationUtil.poisson(interArrivalDelay)), interArrivalDelay);
